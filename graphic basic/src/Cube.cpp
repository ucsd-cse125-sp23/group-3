...
void Cube::spin(glm::vec3 deg) {
    // Update the model matrix by multiplying a rotation matrix
    model = model * glm::rotate(glm::radians(deg.z), glm::vec3(0.0f, 0.0f, 1.0f));
    model = model * glm::rotate(glm::radians(deg.y), glm::vec3(0.0f, 1.0f, 0.0f));
    model = model * glm::rotate(glm::radians(deg.x), glm::vec3(1.0f, 0.0f, 0.0f));
}